...
class CartPoleQLearningAgent:
    ...
    def __init__(self, alpha=0.2, disFactor=1.0, explorRate=0.5, explorDecay_rate=0.99):
        self.alpha = alpha
        self.disFactor = disFactor
        self.explorRate = explorRate
        self.explorDecay_rate = explorDecay_rate
        self.state = None
        self.action = None
    ...
    def begin_episode(self, observation):
        ...
        self.explorRate *= self.explorDecay_rate
    ...
    def act(self, observation, reward):
        ...
        enable_exploration = (1 - self.explorRate) <= np.random.uniform(0, 1)
        ...
        self.q[self.state, self.action] += self.alpha * \
            (reward + self.disFactor * max(self.q[next_state, :]) - self.q[self.state, self.action])
...
class EpisodeHistory:
    ...
    def __init__(self, capacity, plot_episode_count=200, max_timesteps_per_episode=200,
                 goal_avg_episode_length=195, goal_consecutive_episodes=100):
